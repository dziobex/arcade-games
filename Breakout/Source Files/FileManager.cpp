...
// get 10 highest records from the file with highscores //
std::string FileManager::GetHighscores() {
...
// set the new record if it's greater than the last, the lowest one from the current highscores //
void FileManager::SetScore(int score) {
	std::ifstream reader("resources/hs.txt");
	Score scores[10];
	std::string date;
	int scoreFile;
...
#pragma warning(suppress : 4996) // vs stop insulting me pls again
...
std::string FileManager::GetFromFile(std::string filename, int separator) {
